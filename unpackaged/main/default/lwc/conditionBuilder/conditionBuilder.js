...
export default class ConditionBuilder extends LightningElement {
    ...
    operations = [
        {
            label: 'equals',
            value: ' = ',
            types: 'String,Picklist,Url,Email,TextArea,Reference,Phone,Date,DateTime,Currency,Double,Boolean,Int,Address,NestedField,RawText'
        },
        {
            label: 'not equals',
            value: ' != ',
        ...
        {label: 'less than', value: ' < ', types: 'Date,DateTime,Currency,Double,Int,Address,NestedField,RawText'},
        {label: 'greater than', value: ' > ', types: 'Date,DateTime,Currency,Double,Int,Address,NestedField,RawText'},
        {label: 'less than or equals', value: ' <= ', types: 'Date,DateTime,Currency,Double,Int,Address,NestedField,RawText'},
        {label: 'greater than or equals', value: ' >= ', types: 'Phone,Date,DateTime,Currency,Double,Int,Address,NestedField,RawText'},
        {label: 'LIKE', value: ' LIKE ', types: 'String,Picklist,Url,Email,Reference,Phone,NestedField,RawText'},
        {
            label: 'NOT IN',
            value: ' NOT IN ',
            types: 'String,Picklist,Url,Email,Reference,Phone,Date,DateTime,Currency,Double,Int,Address,NestedField,RawText'
        },
        {
            label: 'IN',
            value: ' IN ',
        ...
        },
        {
            label: 'INCLUDES',
            value: ' INCLUDES ',
            types: 'MultiPicklist,NestedField,RawText'
        },
        {
            label: 'EXCLUDES',
            value: ' EXCLUDES ',
    ...
    fieldTypeSettings = {
        ...
        RawText: {inputType: 'text', dataTransformationFunction: 'wrapInQuotes'},
        MultiPicklist: {inputType: 'text', dataTransformationFunction: 'wrapInBrackets'}
    };
    ...
    parseWhereClause(value) {
        if (value) {
            const matcher = new RegExp('(( ?(OR|AND) ?))(?!\\w)|\w*(?<!INCLUDES) [()]', 'g');
            ...
            let conditions = value.replace(new RegExp(' OR| AND', 'g'), '|||').split('|||');
        ...
        conditions.forEach(curCondition => {
            for (let i = 0; i < this.operations.length; i++) {
                ...
                if (curCondition.indexOf(curLogicType.value) !== -1) {
                    ...
                    newCondition.fieldName = conditionParts[0].replace('(', '').trim(); //When custom logic is set, e.g., 1 AND (2 OR 3), the field name of #2 will look like: (FieldName -> Need to remove (
                    newCondition.operation = curLogicType.value;
                    if (fieldDescriptor) {
                        if (this.fieldTypeSettings[fieldDescriptor.dataType]) {
                            if(this.fieldTypeSettings[fieldDescriptor.dataType].dataTransformationFunction === 'wrapInQuotes'){
                                if(conditionParts[1].trim().endsWith('\' )')){
                                    // Occurs when custom logic is set, e.g., 1 AND (2 OR 3)
                                    // If condition #3 is text, its value will look like: 'test')
                                    // Thus, must remove the last )
                                    newCondition.value = this.stripQuotes(conditionParts[1].slice(0, -1).trim());
                                }
                                else {
                                    newCondition.value = this.stripQuotes(conditionParts[1].trim());
                                }
                            }
                            else if(this.fieldTypeSettings[fieldDescriptor.dataType].dataTransformationFunction === 'wrapInBrackets') {
                                newCondition.value = this.stripBrackets(conditionParts[1].trim());
                            }
                            else {
                                newCondition.value =  conditionParts[1].trim();
                            }
                        }
                        newCondition.dataType = fieldDescriptor.dataType;
                    }
                    else if(fieldDescriptor == undefined && newCondition.operation.includes('INCLUDES')) {
                        newCondition.value = this.stripBrackets(conditionParts[1].trim());
                        newCondition.dataType = 'MultiPicklist';
                    }
                    else if(fieldDescriptor == undefined && conditionParts[1].trim().includes('\'')){
                        if(this.fieldTypeSettings['RawText'].dataTransformationFunction === 'wrapInQuotes'){
                            if(conditionParts[1].trim().endsWith('\' )')){
                                // Occurs when custom logic is set, e.g., 1 AND (2 OR 3)
                                // If condition #3 is text, its value will look like: 'test')
                                // Thus, must remove the last )
